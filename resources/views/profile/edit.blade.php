            ...
            <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
                ...
                <section>
                    ...
                    <x-splade-form method="patch" :action="route('profile.update')" :default="$user" class="mt-6 space-y-6" preserve-scroll>
                        <x-splade-select name="crop_id" :options="$crops" option-label="hidroponik" option-value="id" :label="__('Pilih Tanaman')"/>
                        <x-splade-input id="crop_amount" name="crop_amount" type="number" :label="__('Jumlah Tanaman')" />
